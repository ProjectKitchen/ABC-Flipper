...
def displayLCD(msg):
    global canvas, device
    #draw=canvas(device)
    with canvas(device) as draw:
        w, h = draw.textsize(text=msg, font=font)
        left = (device.width - w) / 2
        top = (device.height - h) / 2
        draw.text((left, top), text=msg, font=font, fill="white")

displayLCD(codes[randrange(len(codes))])
...
def createScene():
    ...
    for i in range (maxLetters):
        ...
        b=tkCanvas.create_rectangle(xpos+letterwidth*i, ypos, xpos+letterwidth*(i+1), y1, fill=boxcolor, outline="white")
        tkCanvas.lift(letterIDs[i],b)
